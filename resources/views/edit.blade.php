...
<main class="form-page">
    <section class="page-header">
        <h1>Edite um Filme</h1>
        <p>Atualize os campos!!</p>
    </section>

    <form action="{{ route('movie.update') }}" method="POST" enctype="multipart/form-data" novalidate>
        @csrf
        @method('PUT')
        ...
        <div class="input-group">
            <div class="input-field @error('year') error @enderror">
                ...
                <input type="number" id="year" name="year" min="1900" max="2100" step="1" value="{{ old('year', $movie->year) }}" required>
        ...
        <div class="input-group">
            <div class="input-field @error('category_id') error @enderror">
                ...
                <select id="category_id" name="category_id" required>
                    @foreach ($categories as $category)
                        <option value="{{ $category['id'] }}" {{ (old('category_id', $movie->category->id) == $category['id']) ? 'selected' : '' }}>
                            {{ $category['name'] }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="input-field @error('link') error @enderror">
                ...
                <input type="url" id="link" name="link" value="{{ old('link', $movie->link) }}">
